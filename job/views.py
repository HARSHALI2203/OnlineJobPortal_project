...
def recruiter_signup(request):
    ...
    if request.method == 'POST':
        ...
        try:
            user = User.objects.create_user(first_name=f, last_name=l, username=e, password=pas)
            Recruiter.objects.create(user=user, mobile=con, image=img, gender=gen,company=company, type="recruiter", status= "pending")
            error = "no"
...
def user_signup(request):
    ...
    if request.method == 'POST':
        ...
        try:
            user = User.objects.create_user(first_name=f, last_name=l, username=e, password=pas)
            StudentUser.objects.create(user=user, mobile=con, image=img, gender=gen, type='student')
            error = "no"
        except Exception as e:
            error = str(e)
    d = {'error': error}
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if u.check_password(c):
                u.set_password(n)  # Corrected the method to set the new password
                u.save()
                error = "no"
            else:
                error = "incorrect"  # Changed 'no' to 'incorrect' to distinguish from successful password change
        except:
            error = "yes"
